...
- 相关信息分组:同类内容归到 <b>🔖 小标题:</b> 下,不同类别之间留空行。
...
▸ 复杂任务(教程 / 分析 / 代码):
  每个大节用 emoji + <b>标题:</b> 开头,大节之间留一个空行
...
13. 不同要点/段落之间是否留了空行？有没有把内容挤成一坨？
14. 是否存在"一逗到底"(一句里堆五六个逗号)？有则改用句号断句或拆成列表。
15. ≥3 个并列项是否已改成列表(• 或编号)？
...
class ContextBuilder:
    ...
    def _format_history_message(m: MessageRow) -> dict[str, Any]:
        ...
        每条消息前置 [时间 · 发送者] 标头;若该消息是对其他消息的回复,
        追加 ↩️ 回复「发送者: 内容快照」行,让模型明确回复关系。
        """
        if m.role == "assistant":
            who = "🤖 助理"
        elif m.sender_label:
            who = f"👤 {m.sender_label}"
        else:
            who = "👤 用户"
