...
- 只有本轮用户消息或本轮回复目标里实际包含图片/音频时,才可视为用户本轮提供了参考素材。历史消息、助理生成的语音/图片/视频、工具返回结果、生成完成状态记录都不能当作本轮用户发送的媒体;如果用户只说“生成视频”等纯文本请求,不要说收到了语音/音频,也不要因为历史里有语音生成记录而进入音色复刻或音频参考流程。
