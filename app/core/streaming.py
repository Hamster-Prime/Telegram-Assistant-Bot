...
class _TickLoopMixin:
    ...
    async def _tick_loop(self, do_edit: Any) -> None:
        ...
        分支:
        1. 有新内容(_pending != _committed):写入纯文本(无光标),并预设下次
           idle 时光标为"亮"(_cursor_on=False → 下次 toggle=True)。写完内容
           后必须为亮。
        2. 内容静默(_committed 非空、无新内容):翻转 _cursor_on 作为后缀闪烁。
        3. 占位阶段(首条内容前):整条消息即光标,▌ ↔ nbsp 交替闪烁。
        """
        try:
